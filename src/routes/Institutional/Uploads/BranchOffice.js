...
export default class Uploads extends Component {
  ...
  downFile = () => {
    ...
    let postData;
    if (this.state.headOffice) postData = 'Head2Template.xlsx';
    else postData = 'Bran2Template.xlsx';
  ...
  upload = () => {
    ...
    this.props.dispatch({
      ...
      name: headOffice ? 'uploadHead2' : 'uploadBranch2'
    }).then(() => {
        ...
        <Spin spinning={this.props.loading}>
          <Card style={{ height: '100%' }}>
            <div>
              ...
              <div style={{ height: 40, lineHeight: '40px' }}>您所需要上传的数据如下：</div>
              <div style={{ padding: 10, marginBottom: 20 }}>

                {headOffice ?
                  <div>
                    <div style={{ marginBottom: 20 }}>（1）业务管理指标：包含客服评价指标、车险管理评价指标、财产险管理评价、人身险管理评价等</div>
                    <div style={{ marginBottom: 20 }}>（2）人力资源效能指标：包含人均产能、后台人均支持保费、后台人力成本保费率等</div>
                    <div style={{ marginBottom: 20 }}>（3）内控与风险指标</div>
                    <div>（4）分公司加减分项</div>
                  </div> :
                  <div>
                    <div style={{ marginBottom: 20 }}>（1）规模能力指标：市场份额、市场增速、分公司增速、保费时间进度达成率等</div>
                    <div>（2）综合成本率指标：综合成本率、保单变动成本率、销售推动费用率、变动管理费用率、固定成本与人力成本占保费比等</div>
                  </div>
                }
