...
import {
  Card, Form, Row, Col, Select, Input, Checkbox, Button, Modal, message, Collapse, Spin,
} from 'snk-web';
...
export default class Query extends PureComponent {
  ...
  componentWillReceiveProps(props) {
    ...
    console.log(props.global.publicMode)
  ...
  submitForm = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      mainDim = 'calc' + values.cycleTime.split('-')[0];
      ...
      for (const i in values.keys) {
        ...
        if (values[`selectDim${values.keys[i]}`] !== undefined
          && values[`selectDim${values.keys[i]}`][1] !== undefined
      ...
      this.props.dispatch({
        ...
        if (getMeasureData.content !== null) {
          setTimeout(() => { // 防止echarts图表数据过大 延长加载时间
            ...
            this.setState({ loadEcharts: false, getMeasureData, downloadButFlag: true });
          }, 2000);
        }
      }).catch((e) => {
  ...
  updateCommaly = (e) => {
    ...
    this.conserveData(searchData, this.state.analyName, 'updateCommaly', 'zhjkquery');
  }
  ...
  onOkModel = (e) => {
    ...
    if (analysisName === '') {
      ...
      message.warn('名称长度请控制在200个字以内');
      return;
    } else {
      this.conserveData(searchData, analysisName, 'addCommaly', 'zhjkquery');
    }
  }
    ...
    this.props.form.validateFields((err, values) => {
      ...
      dispatch({
        ...
        payload: {
          ...
          appId,
          appName: '自动核保',
          themeName: this.state.themeId === 'zhindexudr' ? '自核指标(核保时间)' : '自核指标(保险起期)',
  ...
  renderForm() {
    ...
    const { getApplicationData, getThemeData, getUserInfoData } = this.props.analysis;
    let { clickCommalyData } = this.props.analysis;
    let { getDimensions, measure, getTimePeriodData, publicMode } = this.state;
    let dimensionArr = [], searchData = [];
    let indexAnalyze = '';
