...
import {
  Card, Form, Row, Col, Select, Input, Button, Modal, message, Collapse, Spin, DatePicker,
} from 'snk-web';
...
export default class BusistructureQuery extends PureComponent {
  state = {
  ...
  componentDidMount() {
    ...
    if (themeId !== '' && themeId === 'busistructure') {
      this.setState({ appName: '车险品质', themeName: '业务结构' });
    }
  ...
  reloadData(data, isPar) {
    if (data.commonAnalysisConfig !== undefined) {
      ...
      if (searchData.mainDim !== undefined && searchData.dimension !== undefined) {
        ...
        this.setState({ mainDimOption: temp });
        setTimeout(() => {
          this.props.form.setFieldsValue({ mainDim: searchData.mainDim });
        }, 300);
      }
      // 回写占比维度
      if (searchData.occupDim !== undefined && searchData.dimension !== undefined) {
        this.setState({ occupDimOption: searchData.occupDim });
        setTimeout(() => {
          this.props.form.setFieldsValue({ occupDim: searchData.occupDim });
        }, 300);
      }
    }
  }
  ...
  changeMainDim = () => {
    ...
    const selectCycle = this.props.form.getFieldValue('cycleTime');
    if (selectCycle !== '' && selectCycle !== undefined) {
      tempMainDimArr.push(`calc${selectCycle.split('-')[0]}` + '-' + `${selectCycle.split('-')[1]}`);
    }
    ...
    this.setState({ mainDimOption: tempMainDimArr });
  }

  changeOccupDim = () => {
    const { getDimensions } = this.props.analysis;
    const occupDimTempArr = [];
    this.props.form.validateFields((err, values) => {
      if (getDimensions.length > 0) {
        for (const i in getDimensions) {
          if (values[`customDim${i}`] !== '' && values[`customDim${i}`] !== undefined) {
    ...
    // 分析维度2个以上的时候 才能能有占比维度的选项
    if (occupDimTempArr.length > 1) {
      this.setState({ occupDimOption: occupDimTempArr });
    } else {
      this.setState({ occupDimOption: [] });
    }
  }
  ...
  submitForm = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      let statitime = {}; // 统计时间
      let timeSegment = [], occupDim = []; // 时间周期 和 占比维度
      let mainDim = '', token = '';
      mainDim = values.mainDim;
      values.occupDim.map((item) => { return occupDim.push(item.split('-')[0]); });
      ...
      this.props.dispatch({
        type: 'analysis/getBusistructureMeasureData',
        payload,
      }).then(() => {
  ...
  saveMode = () => {
    this.props.form.validateFields((err, values) => {
      ...
      const { saveMode, themeId, appId, publicMode, appName, themeName } = this.state;
  ...
  setPostData = (searchData, values) => {
    ...
    let statitime = {}; // 统计时间
    let mainDim = ''; //主维度
    let filter = [], dimension = [], measure = [], measureItem = [], occupDim = []; // 过滤器、选择维度和指标、占比维度
    statitime = this.getStatiTime(values, 2);
    mainDim = values.mainDim;
    occupDim = values.occupDim;
  ...
  returnData = () => {
    ...
    this.props.dispatch({
      type: 'analysis/getBusistructureRetMeasureData',
      payload,
    }).then(() => {
  ...
  getDimensionAndMainDim = (getRetMeasureData, getDimensions) => {
    let temp = [], dimension = [], occupDim = [];
    let mainDimName = '', index = '', mainDim = '', occupDimName = '';
    ...
    // temp 是当前分析维度选择的层级
    temp.map((item) => {
      // 如果它事主维度的话 拿取维度名称 mainDimName 然后通过它改变最新层级元素
    ...
    for (const i in getDimensions) {
      ...
      const occupDimItem = this.props.form.getFieldValue(`occupDim`);
      occupDimItem.map((item) => {
        const mainDim = this.props.form.getFieldValue('mainDim');
        if (item.split('-')[0] === mainDim) {
          occupDimName = item.split('-')[1];
        } else {
          occupDim.push(item.split('-')[0]);
        }
      });
      if (getDimensions[i].dimTotalName === occupDimName) {
        if (getRetMeasureData.mainDim !== undefined && getRetMeasureData.mainDim !== '') {
          getDimensions[i].tableDesc.map((item) => {
            if (item.columnValue === getRetMeasureData.mainDim) {
              const value = item.columnValue;
  ...
  renderForm() {
    ...
    let dimensionArr = [], mainDimArr = [], searchData = [], occupDimOptionArr = [];
    let indexAnalyze = '';
    console.log([...new Set(this.state.occupDimOption)])
    if (clickCommalyData !== undefined && clickCommalyData.commonAnalysisConfig !== undefined) {
      searchData = JSON.parse(clickCommalyData.commonAnalysisConfig);
    }
    [...new Set(this.state.mainDimOption)].map((item) => {
      const optionItem = (
        <Option value={item.split('-')[0]} key={item.split('-')[0]}>
          {item.split('-')[1]}
        </Option>
      );
      mainDimArr.push(optionItem);
    });

    [...new Set(this.state.occupDimOption)].map((item) => {
      const optionItem = (
        <Option value={item.split('-')[0] + '-' + item.split('-')[1]} key={item.split('-')[0]}>
          {item.split('-')[1]}
        </Option>
      );
      occupDimOptionArr.push(optionItem);
    });
    ...
    return (
      <Form layout="inline">
        <Collapse defaultActiveKey={['1', '2', '3', '4', '5', '6']}>
          ...
          <Panel header="占比维度" key="5">
            <Spin spinning={this.state.onLoading}>
              <FormItem label='占比维度元素'>
                {getFieldDecorator('occupDim', { initialValue: [], })(
                  <Select style={{ width: 398, marginRight: 8 }} onFocus={this.changeOccupDim} mode="multiple">
                    {occupDimOptionArr.length > 0 ? occupDimOptionArr.map(i => { return i }) : ''}
                  </Select>
                )}
              </FormItem>
            </Spin>
            ...
            <Spin spinning={this.state.onLoading}>
              <FormItem label='主维度元素'>
                {getFieldDecorator('mainDim', {
                  ...
                  <Select style={{ width: 140, marginRight: 8 }} onFocus={this.changeMainDim} allowClear>
                    {mainDimArr.length > 0 ? mainDimArr.map(i => { return i }) : ''}
                  </Select>
                )}
              </FormItem>
            </Spin>
  ...
  render() {
    const { measure } = this.props.analysis;
    const { mainDimOption, mainDim, getMeasureData } = this.state;
          ...
          <div>
            ...
            <Spin spinning={this.state.loadEcharts}>
              <ShowEcharts
                ...
                measureData={measure}
                selectMainDimArr={mainDimOption}
                mainDim={mainDim}
