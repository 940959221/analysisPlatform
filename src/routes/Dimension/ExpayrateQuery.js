...
export default class ExpayrateQuery extends PureComponent {
  state = {
    ...
    if (data.commonAnalysisConfig !== undefined) {
      ...
      if (searchData.mainDim !== undefined && searchData.dimension !== undefined) {
        ...
        if (selectCycle !== '' && selectCycle !== undefined) {
          temp.push(`calc${selectCycle.split('-')[0]}` + '-' + `统计时间`);
        }
  ...
  submitForm = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      let statitime = {}, endtime = {}; // 统计时间 截至时间
      let timeSegment = [];
      let mainDim = '', token = '';
      ...
      if (values.timeContent.length > 0) {
        endtime['timeContent'] = values.timeContent;
      } else {
        message.warn('请选择具体的时间节点');
        return;
      }
      // 获取指标
      ...
      this.props.dispatch({
        type: 'analysis/getExpayrateMeasureData',
        payload,
      }).then(() => {
  ...
  returnData = () => {
    ...
    this.props.dispatch({
      type: 'analysis/getRetExpayrateMeasureData',
      payload,
    }).then(() => {
