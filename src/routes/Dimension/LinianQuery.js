...
export default class LinianQuery extends PureComponent {
  state = {
  ...
  reloadData(data, isPar) {
    if (data.commonAnalysisConfig !== undefined) {
      ...
      // 回显截止时间
      if (searchData.endtime !== undefined) {
        const temp = [];
        searchData.endtime.timeContent.map((item) => {
          temp.push(item);
        });
        this.props.form.setFieldsValue({ endtime: `${searchData.endtime.cycle}`, timeContent: temp });
        if (searchData.endtime.cycle !== undefined) {
          const cycle = searchData.endtime.cycle;
          // 获取当前截止周期的时间
          this.props.dispatch({
      ...
      if (searchData.mainDim !== undefined && searchData.dimension !== undefined) {
        let temp = [];
        const selectEndtime = this.props.form.getFieldValue('endtime');
        if (selectEndtime !== '' && selectEndtime !== undefined) {
          temp.push(`datadate` + '-' + `时间节点`);
        }
            ...
            for (var i = 0; i < keys.length; i++) {
              ...
              if (value === item) {
                ...
                if (this.state.filterObj.length > 0) {
                  ...
                  for (var j = 0; j < this.state.filterObj.length; j++) { // 相同维度的元素去掉低层级元素 留下最高级元素
  ...
  submitForm = (e) => {
    ...
    console.log(queryConfigParam);
    console.log(commonAnalysisConfig);
    this.props.form.validateFields((err, values) => {
      if (err) return;
      let searchData = [];
      const { clickCommalyData } = this.props.analysis;
      ...
      let isQuery = '1';
      let dimension = [], measure = [], filter = {}, customDimension = {}; // 选择维度 指标 自定义维度
      let endtime = {}; // 截止时间
      let timeSegment = [];
      let mainDim = '', token = '';
      ...
      // 截至时间
      endtime['cycle'] = values.endtime;
      endtime['timeContent'] = values.timeContent;
      // 获取指标
      ...
      this.props.dispatch({
        type: 'analysis/getLinianMeasureData',
        payload,
      }).then(() => {
  ...
  getEndTimeContent = (value) => {
    this.props.form.setFieldsValue({ timeContent: [], });
    const payload = {
      appId: this.state.appId,
      themeId: this.state.themeId,
      cycle: value,
  ...
  returnData = () => {
    ...
    this.props.dispatch({
      type: 'analysis/getLinianRetMeasureData',
      payload,
    }).then(() => {
  ...
  renderForm() {
    ...
    const { getApplicationData, getThemeData, getUserInfoData, getDimensionContentData, getEndTimePeriodData, getEndTimeContentData } = this.props.analysis;
    let { clickCommalyData } = this.props.analysis;
    let { getDimensions, measure, dimension, publicMode } = this.state;
    let dimensionArr = [], tempArr = [], searchData = [], getEndTimeArr = [], getEndTimeContent = [];
    let indexAnalyze = '';
    ...
    //   <Col>
    //     <FormItem label="统计周期">
    //       {getFieldDecorator('cycleTime',{rules: [{ required: true, message: '必选' }]
    //     })(
    //         <Select style={{ width: 80, marginRight: 8 }} onSelect={this.selectTime} allowClear >
    ...
    getEndTimePeriodData.map((item) => {
      const optionItem = (
        <Option value={item.periodId} key={item.periodId}>
          {item.periodName}
        </Option>
      );
      getEndTimeArr.push(optionItem);
    });

    getEndTimeContentData.map((item) => {
      getEndTimeContent.push(<Option key={item}>{item}</Option>)
    });

    return (
      <Form layout="inline">
        <Collapse defaultActiveKey={['1', '2', '3', '4', '5', '6']}>
          <Panel header="时间节点" key="1">
            <span style={{
              color: '#f5222d', fontSize: 12, fontFamily: 'SimSun', margin: '8px 4px 0 0', display: 'inline-block'
            }}
            >*
            </span>
            <FormItem label=''>
              ...
              {getFieldDecorator('timeContent', {
                ...
                <Select
                  disabled={this.props.form.getFieldValue('endtime') === '' || this.props.form.getFieldValue('endtime') === undefined}
                  mode="multiple"
                  placeholder="请选择"
                  style={{ width: 400 }} >
                  {getEndTimeContent}
