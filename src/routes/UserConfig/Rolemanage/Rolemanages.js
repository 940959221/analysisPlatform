...
export default class Components extends Component {
    ...
    const columns = [
      {
        ...
        render: (text, record, index) => {
          return (
            <span>
              <Link to={{pathname: '/userconfig/rolemanage/creatRolemanages', id: record.key}}>编辑</Link>
              <span>
  ...
  setTable = (payload) =>{
    this.props.dispatch({
      type: 'analysis/qRoleInfoLis',
      payload
    }).then(() => {
      const { qRoleInfoLisData: { list, total, pageSize, pageNum } } = this.props.analysis;
      const data = [];
      for(let i of list){
        data.push({
          key: i.roleId,
  ...
  query = () => {
    ...
    this.setTable(obj);
    // 在查询的请求发送完之后修改值
    this.setState({ roleName: values.roleName });
  }
                ...
                <Col>
                  ...
                  <div style={{display: 'flex', justifyContent: 'flex-end', margin: '20px 0'}}>
                    <Button style={{marginRight: 15}} type='primary'>
                      <Link to={{pathname: '/userconfig/rolemanage/creatRolemanages'}}>新建</Link>
                    </Button>
